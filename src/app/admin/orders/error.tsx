...
export default function AdminOrdersError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error("Admin orders error caught:", error);
  }, [error]);
      ...
      <div className="bg-card border border-border rounded-xs shadow-lg max-w-md w-full p-8 text-center space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            {error.message || "Something went wrong while loading the customer orders."}
          </p>
        </div>
